...
#Propagator order 8 
function Prop_order8_MB1000(du,u,p,t)

    du[1] = (p[1] + p[2] - 1) * u[1] - (p[1] + 3 * p[2]) * (u[1] ^ 2) + 2 * p[2] *( u[1] ^ 3)
    #Orden 1
    for j in 2:m1+1
        du[j] = (p[1] + p[2] - 1) * u[j] - (p[1] + 3 * p[2]) * (u[j] ^ 2) + 2 * p[2] * (u[j] ^ 3) + sqrt(2/T) * sin(((j-1) * pi * t)/ T) * p[2] * u[1] * (1- u[1])
    end
    ##Orden 2 mixed
    du[m1+2] = (p[1] + p[2] - 1) * u[m1+2] - (p[1] + 3 * p[2]) * (u[m1+2] ^ 2) + 2 * p[2] *( u[m1+2] ^ 3) + sqrt(2/T) * p[2] * (sin(((2-1) * pi * t)/ T) * u[3] * (1- u[3]) + sin(((3-1) * pi * t)/ T) * u[2] * (1- u[2]))
    du[m1+3] = (p[1] + p[2] - 1) * u[m1+3] - (p[1] + 3 * p[2]) * (u[m1+3] ^ 2 )+ 2 * p[2] *( u[m1+3] ^ 3) + p[2] * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[4] * (1- u[4]) + sin(((4-1) * pi * t)/ T) * u[2] * (1- u[2]))
    du[m1+4] = (p[1] + p[2] - 1) * u[m1+4] - (p[1] + 3 * p[2]) * (u[m1+4] ^ 2 )+ 2 * p[2] *( u[m1+4] ^ 3) + p[2] * sqrt(2/T) * (sin(((3-1) * pi * t)/ T) * u[4] * (1- u[4]) + sin(((4-1) * pi * t)/ T) * u[3] * (1- u[3]))
    ##Orden 2 diagonal
    du[m1+5] = (p[1] + p[2] - 1) * u[m1+5] - (p[1] + 3 * p[2]) * (u[m1+5] ^ 2) + 2 * p[2] * (u[m1+5] ^ 3) + p[2] * sqrt(2) *sqrt(2/T) * sin(((2-1) * pi * t)/ T) * u[2] * (1- u[2])
    du[m1+6] = (p[1] + p[2] - 1) * u[m1+6] - (p[1] + 3 * p[2]) * (u[m1+6] ^ 2) + 2 * p[2] * (u[m1+6] ^ 3) + p[2] * sqrt(2) *sqrt(2/T) * sin(((3-1) * pi * t)/ T) * u[3] * (1- u[3])
    #Orden 3 mixed
    du[m1+7] = (p[1] + p[2] - 1) * u[m1+7] - (p[1] + 3 * p[2]) * (u[m1+7] ^ 2) + 2 * p[2] * (u[m1+7] ^ 3) + p[2] * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[m1+6] * (1- u[m1+6]) + sqrt(2)* sin(((3-1) * pi * t)/ T) * u[m1+2] * (1- u[m1+2]))
    du[m1+8] = (p[1] + p[2] - 1) * u[m1+8] - (p[1] + 3 * p[2]) * (u[m1+8] ^ 2 )+ 2 * p[2] * (u[m1+8] ^ 3) + p[2] * sqrt(2/T) * (sqrt(2) * sin(((2-1) * pi * t)/ T) * u[m1+2] * (1- u[m1+2]) + sqrt(1)* sin(((3-1) * pi * t)/ T) * u[m1+5] * (1- u[m1+5]))
    ##Orden 3 diagonal
    du[m1+9] = (p[1] + p[2] - 1) * u[m1+9] - (p[1] + 3 * p[2]) * (u[m1+9] ^ 2 )+ 2 * p[2] * (u[m1+9] ^ 3) + p[2] * sqrt(2/T) * (sqrt(3) * sin(((2-1) * pi * t)/ T) * u[m1+5] * (1- u[m1+5]))
    du[m1+10] = (p[1] + p[2] - 1) * u[m1+10] - (p[1] + 3 * p[2]) * (u[m1+10] ^ 2) + 2 * p[2] *( u[m1+10] ^ 3) + p[2] * sqrt(2/T) * (sqrt(3) * sin(((3-1) * pi * t)/ T) * u[m1+6] * (1- u[m1+6]))
    #Orden 4
    du[m1+11] = (p[1] + p[2] - 1) * u[m1+11] - (p[1] + 3 * p[2]) * (u[m1+11] ^ 2) + 2 * p[2] * (u[m1+11] ^ 3) + p[2] * sqrt(2/T) * (sqrt(4) * sin(((2-1) * pi * t)/ T) * u[m1+9] * (1- u[m1+9]))
    du[m1+12] = (p[1] + p[2] - 1) * u[m1+12] - (p[1] + 3 * p[2]) * (u[m1+12] ^ 2 )+ 2 * p[2] * (u[m1+12] ^ 3) + p[2] * sqrt(2/T) * (sqrt(4) * sin(((3-1) * pi * t)/ T) * u[m1+10] * (1- u[m1+10]))
    #Orden 5
    du[m1+13] = (p[1] + p[2] - 1) * u[m1+12] - (p[1] + 3 * p[2]) * (u[m1+13] ^ 2) + 2 * p[2] * (u[m1+13] ^ 3) + p[2] * sqrt(2/T) * (sqrt(5) * sin(((2-1) * pi * t)/ T) * u[m1+11] * (1- u[m1+11]))
    du[m1+14] = (p[1] + p[2] - 1) * u[m1+14] - (p[1] + 3 * p[2]) * (u[m1+14] ^ 2) + 2 * p[2] * (u[m1+14] ^ 3) + p[2] * sqrt(2/T) * (sqrt(5) * sin(((3-1) * pi * t)/ T) * u[m1+12] * (1- u[m1+12]))
    #Orden 6
    du[m1+15] = (p[1] + p[2] - 1) * u[m1+15] - (p[1] + 3 * p[2]) * (u[m1+15] ^ 2) + 2 * p[2] * (u[m1+15] ^ 3) + p[2] * sqrt(2/T) * (sqrt(6) * sin(((2-1) * pi * t)/ T) * u[m1+13] * (1- u[m1+13]))
    du[m1+16] = (p[1] + p[2] - 1) * u[m1+16] - (p[1] + 3 * p[2]) * (u[m1+16] ^ 2) + 2 * p[2] * (u[m1+16] ^ 3) + p[2] * sqrt(2/T) * (sqrt(6) * sin(((3-1) * pi * t)/ T) * u[m1+14] * (1- u[m1+14]))
    #Orden 7
    du[m1+17] = (p[1] + p[2] - 1) * u[m1+17] - (p[1] + 3 * p[2]) * (u[m1+17] ^ 2) + 2 * p[2] * (u[m1+17] ^ 3) + p[2] * sqrt(2/T) * (sqrt(7) * sin(((2-1) * pi * t)/ T) * u[m1+15] * (1- u[m1+15]))
    du[m1+18] = (p[1] + p[2] - 1) * u[m1+18] - (p[1] + 3 * p[2]) * (u[m1+18] ^ 2 )+ 2 * p[2] * (u[m1+18] ^ 3) + p[2] * sqrt(2/T) * (sqrt(7) * sin(((3-1) * pi * t)/ T) * u[m1+16] * (1- u[m1+16]))
    #Orden 8
    du[m1+19] = (p[1] + p[2] - 1) * u[m1+19] - (p[1] + 3 * p[2]) * (u[m1+19] ^ 2) + 2 * p[2] * (u[m1+19] ^ 3) + p[2] * sqrt(2/T) * (sqrt(8) * sin(((2-1) * pi * t)/ T) * u[m1+17] * (1- u[m1+17]))
    du[m1+20] = (p[1] + p[2] - 1) * u[m1+20] - (p[1] + 3 * p[2]) * (u[m1+20] ^ 2) + 2 * p[2] * (u[m1+20] ^ 3) + p[2] * sqrt(2/T) * (sqrt(8) * sin(((3-1) * pi * t)/ T) * u[m1+18] * (1- u[m1+18]))
...
function solution1(Data1,M)
    ...
    for k in 1:M # Loop for trajectories
        ...
        #order 2 diagonal 
        Xi[m1+5:m1+6]= ((z1[1:2].^2).-1) 
        #order 3 mixed 
        Xi[m1+7]= sqrt(6)*(z1[1]) * ((z1[2]) ^ 2 - 1) /sqrt(2)
        Xi[m1+8]= sqrt(6)*(((z1[1]) ^ 2 - 1) * (z1[2]) )/sqrt(2)
        #order 3 diagonal
        Xi[m1+9:m1+10]= (z1[1:2]).*(((z1[1:2].^2).-3) )
        #order 4 diagonal
        Xi[m1+11:m1+12]=  ((z1[1:2].^4).-(6*(z1[1:2].^2)).+3) 
        #order 5 diagonal
        Xi[m1+13:m1+14]= (z1[1:2].*(-(10*(z1[1:2].^2)).+(z1[1:2].^4).+15))
        #order 6 diagonal
        Xi[m1+15:m1+16]= (45*(z1[1:2].^2).-15*(z1[1:2].^4).+(z1[1:2].^6).-15)
        #order 7 diagonal
        Xi[m1+17:m1+18]= (-105*z1[1:2].+105*(z1[1:2].^3).-21*(z1[1:2].^5).+(z1[1:2].^7))
        #order 8 diagonal
        Xi[m1+19:m1+20]= (-(420*(z1[1:2].^2)).+(210*(z1[1:2].^4)).-28*(z1[1:2].^6).+(z1[1:2].^8).+105)
...
gradientdescentv1([0.52,sigma_est],4,data_1,obs1,M,u0,Time)
