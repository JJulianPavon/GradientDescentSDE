...
ode_func <- function(t, state, parms){
  with(as.list(c(state, parms)),{
    ...
    #Orden 1
    for(j in 2:m1+1){
      du[j] <- p1 * u[j] * (1 - u[j]) +  sqrt(2/T) * sin(((j-1) * pi * t)/ T) * p2 * u[1]
    }
    ##Orden 2 mixed
    du[m1+2] <-  p1 * u[m1+2] * (1 - u[m1+2]) + p2 * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[3]  + sin(((3-1) * pi * t)/ T) * u[2])
    du[m1+3] <-  p1 * u[m1+3] * (1 - u[m1+3]) + p2 * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[4]  + sin(((4-1) * pi * t)/ T) * u[2])
    du[m1+4] <-  p1 * u[m1+4] * (1 - u[m1+4]) + p2 * sqrt(2/T) * (sin(((3-1) * pi * t)/ T) * u[4]  + sin(((4-1) * pi * t)/ T) * u[3])
    ##Orden 2 diagonal
    du[m1+5] <-  p1 * u[m1+5] * (1 - u[m1+5]) + p2 * sqrt(2) *sqrt(2/T) * sin(((2-1) * pi * t)/ T) * u[2]
    du[m1+6] <-  p1 * u[m1+6] * (1 - u[m1+6]) + p2 * sqrt(2) *sqrt(2/T) * sin(((3-1) * pi * t)/ T) * u[3]
    #Orden 3 mixed
    du[m1+7] <- p1 * u[m1+7]* (1 - u[m1+7]) + p2 * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[m1+6]  + sqrt(2)* sin(((3-1) * pi * t)/ T) * u[m1+2])
    du[m1+8] <- p1 * u[m1+8]* (1 - u[m1+8]) + p2 * sqrt(2/T) * (sqrt(2) * sin(((2-1) * pi * t)/ T) * u[m1+2]   + sqrt(1)* sin(((3-1) * pi * t)/ T) * u[m1+5])
    ##Orden 3 diagonal
    du[m1+9] <- p1 * u[m1+9]* (1 - u[m1+9]) + p2 * sqrt(2/T) * (sqrt(3) * sin(((2-1) * pi * t)/ T) * u[m1+5])
    du[m1+10] <- p1 * u[m1+10]* (1 - u[m1+10]) + p2 * sqrt(2/T) * (sqrt(3) * sin(((3-1) * pi * t)/ T) * u[m1+6])
    #Orden 4
    du[m1+11] <- p1 * u[m1+11]* (1 - u[m1+11]) + p2 * sqrt(2/T) * (sqrt(4) * sin(((2-1) * pi * t)/ T) * u[m1+9])
    du[m1+12] <- p1 * u[m1+12]* (1 - u[m1+12]) + p2 * sqrt(2/T) * (sqrt(4) * sin(((3-1) * pi * t)/ T) * u[m1+10])
    #Orden 5
    du[m1+13] <- p1 * u[m1+13]* (1 - u[m1+13]) + p2 * sqrt(2/T) * (sqrt(5) * sin(((2-1) * pi * t)/ T) * u[m1+11])
    du[m1+14] <- p1 * u[m1+14]* (1 - u[m1+14]) + p2 * sqrt(2/T) * (sqrt(5) * sin(((3-1) * pi * t)/ T) * u[m1+12])
    #Orden 6
    du[m1+15] <- p1 * u[m1+15]* (1 - u[m1+15]) + p2 * sqrt(2/T) * (sqrt(6) * sin(((2-1) * pi * t)/ T) * u[m1+13])
    du[m1+16] <- p1 * u[m1+16]* (1 - u[m1+16]) + p2 * sqrt(2/T) * (sqrt(6) * sin(((3-1) * pi * t)/ T) * u[m1+14])
    #Orden 7
    du[m1+17] <- p1 * u[m1+17]* (1 - u[m1+17]) + p2 * sqrt(2/T) * (sqrt(7) * sin(((2-1) * pi * t)/ T) * u[m1+15])
    du[m1+18] <- p1 * u[m1+18]* (1 - u[m1+18]) + p2 * sqrt(2/T) * (sqrt(7) * sin(((3-1) * pi * t)/ T) * u[m1+16])
    #Orden 8
    du[m1+19] <- p1 * u[m1+19]* (1 - u[m1+19]) + p2 * sqrt(2/T) * (sqrt(8) * sin(((2-1) * pi * t)/ T) * u[m1+17])
    du[m1+20] <-  p1 * u[m1+20]* (1 - u[m1+20]) + p2 * sqrt(2/T) * (sqrt(8) * sin(((3-1) * pi * t)/ T) * u[m1+18])
...
gradientcalcu(obs1,100,state,times,0.8,sigma_est)
