...
function Prop_order8_MB1000(du,u,p,t)
    ...
    #Orden 1
    for j in 2:m1+1
        du[j] = p[1] * u[j] * (1 - u[j]) +  sqrt(2/T) * sin(((j-1) * pi * t)/ T) * p[2] * u[1]
    end
    ##Orden 2 mixed
    du[m1+2] = p[1] * u[m1+2] * (1 - u[m1+2]) + p[2] * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[3]  + sin(((3-1) * pi * t)/ T) * u[2])
    du[m1+3] = p[1] * u[m1+3] * (1 - u[m1+3]) + p[2] * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[4]  + sin(((4-1) * pi * t)/ T) * u[2])
    du[m1+4] = p[1] * u[m1+4] * (1 - u[m1+4]) + p[2] * sqrt(2/T) * (sin(((3-1) * pi * t)/ T) * u[4]  + sin(((4-1) * pi * t)/ T) * u[3])
    ##Orden 2 diagonal
    du[m1+5] = p[1] * u[m1+5] * (1 - u[m1+5]) + p[2] * sqrt(2) *sqrt(2/T) * sin(((2-1) * pi * t)/ T) * u[2]
    du[m1+6] = p[1] * u[m1+6] * (1 - u[m1+6]) + p[2] * sqrt(2) *sqrt(2/T) * sin(((3-1) * pi * t)/ T) * u[3]
    #Orden 3 mixed
    du[m1+7] = p[1] * u[m1+7]* (1 - u[m1+7]) + p[2] * sqrt(2/T) * (sin(((2-1) * pi * t)/ T) * u[m1+6]  + sqrt(2)* sin(((3-1) * pi * t)/ T) * u[m1+2])
    du[m1+8] = p[1] * u[m1+8]* (1 - u[m1+8]) + p[2] * sqrt(2/T) * (sqrt(2) * sin(((2-1) * pi * t)/ T) * u[m1+2]   + sqrt(1)* sin(((3-1) * pi * t)/ T) * u[m1+5])
    ##Orden 3 diagonal
    du[m1+9] = p[1] * u[m1+9]* (1 - u[m1+9]) + p[2] * sqrt(2/T) * (sqrt(3) * sin(((2-1) * pi * t)/ T) * u[m1+5])
    du[m1+10] = p[1] * u[m1+10]* (1 - u[m1+10]) + p[2] * sqrt(2/T) * (sqrt(3) * sin(((3-1) * pi * t)/ T) * u[m1+6])
    #Orden 4
    du[m1+11] = p[1] * u[m1+11]* (1 - u[m1+11]) + p[2] * sqrt(2/T) * (sqrt(4) * sin(((2-1) * pi * t)/ T) * u[m1+9])
    du[m1+12] = p[1] * u[m1+12]* (1 - u[m1+12]) + p[2] * sqrt(2/T) * (sqrt(4) * sin(((3-1) * pi * t)/ T) * u[m1+10])
    #Orden 5
    du[m1+13] = p[1] * u[m1+13]* (1 - u[m1+13]) + p[2] * sqrt(2/T) * (sqrt(5) * sin(((2-1) * pi * t)/ T) * u[m1+11])
    du[m1+14] = p[1] * u[m1+14]* (1 - u[m1+14]) + p[2] * sqrt(2/T) * (sqrt(5) * sin(((3-1) * pi * t)/ T) * u[m1+12])
    #Orden 6
    du[m1+15] = p[1] * u[m1+15]* (1 - u[m1+15]) + p[2] * sqrt(2/T) * (sqrt(6) * sin(((2-1) * pi * t)/ T) * u[m1+13])
    du[m1+16] = p[1] * u[m1+16]* (1 - u[m1+16]) + p[2] * sqrt(2/T) * (sqrt(6) * sin(((3-1) * pi * t)/ T) * u[m1+14])
    #Orden 7
    du[m1+17] = p[1] * u[m1+17]* (1 - u[m1+17]) + p[2] * sqrt(2/T) * (sqrt(7) * sin(((2-1) * pi * t)/ T) * u[m1+15])
    du[m1+18] = p[1] * u[m1+18]* (1 - u[m1+18]) + p[2] * sqrt(2/T) * (sqrt(7) * sin(((3-1) * pi * t)/ T) * u[m1+16])
    #Orden 8
    du[m1+19] = p[1] * u[m1+19]* (1 - u[m1+19]) + p[2] * sqrt(2/T) * (sqrt(8) * sin(((2-1) * pi * t)/ T) * u[m1+17])
    du[m1+20] = p[1] * u[m1+20]* (1 - u[m1+20]) + p[2] * sqrt(2/T) * (sqrt(8) * sin(((3-1) * pi * t)/ T) * u[m1+18])
end
...
gradientdescentv1([0.57,sigma_est],5,data_1,obs1,M,u0,Time)
